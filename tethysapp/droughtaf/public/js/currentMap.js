let sldasCurrent = [{
    ...
    CustomLegend: [
        // {color: "#FFFFFF", quantity: "-9999", label: ["No Data"], opacity: "0"},
        {color: "#b10026", quantity: "273", label: ["less than 0°C", ""], opacity: "1"},
        {color: "#fc4e2a", quantity: "278", label: ["0°C to 5°C", ""], opacity: "1"},
        {color: "#feb24c", quantity: "283", label: ["5°C to 10°C", ""], opacity: "1"},
        {color: "#ffeda0", quantity: "288", label: ["10°C to 15°C", ""], opacity: "1"},
        {color: "#c7e9b4", quantity: "293", label: ["15°C to 20°C", ""], opacity: "1"},
        {color: "#7fcdbb", quantity: "298", label: ["20°C to 25°C", ""], opacity: "1"},
        {color: "#1d91c0", quantity: "403", label: ["25°C to 30°C", ""], opacity: "1"},
        {color: "#0c2c84", quantity: "500", label: ["30°C or more", ""], opacity: "1"},
    ]
}, {
...
app.addMonthsLayers = async function () {
    ...
    let selectedMonthFile = sortedMonth.slice(-1)[0];
    app.SLDASYearMonth['year__mm'] = selectedMonthFile.slice(6, 10);
    app.SLDASYearMonth['month__mm'] = selectedMonthFile.slice(10, 12);
    let YearForWMSSelection = parseInt(app.SLDASYearMonth['year__mm']);
    let range = '';
    if (parseInt(app.SLDASYearMonth['month__mm']) < 10) {
        YearForWMSSelection = YearForWMSSelection - 1;
        range = parseInt(app.SLDASYearMonth['month__mm']) + 3;
    } else {
        range = parseInt(app.SLDASYearMonth['month__mm']) - 9;
    }
    app.SLDASYearMonth['range__mm'] = range;
    let selectedMonthFileList = [];
    sortedMonth.forEach(function (value) {
        let currentYear = parseInt(value.slice(6, 10));
        if (currentYear >= YearForWMSSelection) {
            if (currentYear == parseInt(app.SLDASYearMonth['year__mm']) - 1) {
                if (value.slice(10, 12) >= 10) {
                    selectedMonthFileList.push(value);
                }
            } else {
                if (parseInt(app.SLDASYearMonth['year__mm']) == YearForWMSSelection) {
                    if (value.slice(10, 12) >= 10) {
                        selectedMonthFileList.push(value);
                    }
                } else {
    ...
    app.SLDASYearMonth['year__mm'] = YearForWMSSelection;
    app.SLDASYearMonth['month__mm'] = 10
    let currentMonthWMS = [];
...
app.addDekadLayers = async function () {
    ...
    let selectedDekadFile = sortedDekadList.slice(-1)[0];
    app.SLDASYearMonth['year__dd'] = selectedDekadFile.slice(6, 10);
    app.SLDASYearMonth['month__dd'] = selectedDekadFile.slice(10, 12);

    //*****************************************************

    let YearForWMSSelection = parseInt(app.SLDASYearMonth['year__dd']);
    let range = '';
    if (parseInt(app.SLDASYearMonth['month__dd']) < 10) {
        YearForWMSSelection = YearForWMSSelection - 1;
        range = parseInt(app.SLDASYearMonth['month__dd']) + 3;
    } else {
        range = parseInt(app.SLDASYearMonth['month__dd']) - 9;
    }
    app.SLDASYearMonth['range__dd'] = range;
    let selectedFileList = [];
    sortedDekadList.forEach(function (value) {
        let currentYear = parseInt(value.slice(6, 10));
        if (currentYear >= YearForWMSSelection) {
            if (currentYear == parseInt(app.SLDASYearMonth['year__dd']) - 1) {
                if (value.slice(10, 12) >= 10) {
                    selectedFileList.push(value);
                }
            } else {
                if (parseInt(app.SLDASYearMonth['year__dd']) == YearForWMSSelection) {
                    if (value.slice(10, 12) >= 10) {
                        selectedFileList.push(value);
                    }
                } else {
    ...
    app.SLDASYearMonth['year__dd'] = YearForWMSSelection;
    app.SLDASYearMonth['month__dd'] = 10
...
app.add3MonthLayers = async function () {
    ...
    let selected3MonthFile = sorted3Months.slice(-1)[0];
    app.SLDASYearMonth['year__3m'] = selected3MonthFile.slice(6, 10);
    app.SLDASYearMonth['month__3m'] = selected3MonthFile.slice(10, 12);
    //*****************************************************************
    let YearForWMSSelection = parseInt(app.SLDASYearMonth['year__3m']);
    let range = '';
    if (parseInt(app.SLDASYearMonth['month__3m']) < 8) {
        YearForWMSSelection = YearForWMSSelection - 1;
        range = parseInt(app.SLDASYearMonth['month__3m']) + 5;
    } else {
        range = parseInt(app.SLDASYearMonth['month__3m']) - 7;
    }
    app.SLDASYearMonth['range__3m'] = range;
    let selectedFileList = [];
    sorted3Months.forEach(function (value) {
        let currentYear = parseInt(value.slice(6, 10));
        if (currentYear >= YearForWMSSelection) {
            if (currentYear == parseInt(app.SLDASYearMonth['year__3m']) - 1) {

                if (value.slice(10, 12) >= 8) {
                    selectedFileList.push(value);
                }
            } else {
                if (parseInt(app.SLDASYearMonth['year__3m']) == YearForWMSSelection) {
                    if (value.slice(10, 12) >= 8) {
                        selectedFileList.push(value);
                    }
                } else {
    ...
    app.SLDASYearMonth['year__3m'] = YearForWMSSelection;
    app.SLDASYearMonth['month__3m'] = 8
    ...
    let dateisoFormatForLevelFormatter = function (isoDate) {
        ...
        let threeMonths = '';
        if (FullMonth === 12) {
            threeMonths = AllMonths[FullMonth].slice(0, 1) + AllMonths[1].slice(0, 1) + AllMonths[2].slice(0, 1)
        } else if (FullMonth === 11) {
            threeMonths = AllMonths[FullMonth].slice(0, 1) + AllMonths[FullMonth + 1].slice(0, 1) + AllMonths[1].slice(0, 1)
        } else {
...
app.addSPIMonthsLayers = async function () {
    ...
    let sortedMonth = currentMonthly.sort();
    let selectedMonthFile = sortedMonth.slice(-1)[0];
    app.SPIYearMonth['year__mm'] = selectedMonthFile.slice(0, 4);
    app.SPIYearMonth['month__mm'] = selectedMonthFile.slice(4, 6);
    let YearForWMSSelection = parseInt(app.SPIYearMonth['year__mm']);
    let range = '';
    if (parseInt(app.SPIYearMonth['month__mm']) < 10) {
        YearForWMSSelection = YearForWMSSelection - 1;
        range = parseInt(app.SPIYearMonth['month__mm']) + 3;
    } else {
        range = parseInt(app.SPIYearMonth['month__mm']) - 9;
    }
    app.SPIYearMonth['range__mm'] = range;
    let selectedMonthFileList = [];
    sortedMonth.forEach(function (value) {
        let currentYear = parseInt(value.slice(0, 4));
        if (currentYear >= YearForWMSSelection) {
            if (currentYear == parseInt(app.SPIYearMonth['year__mm']) - 1) {
                if (value.slice(4, 6) >= 10) {
                    selectedMonthFileList.push(value);
                }
            } else {
                if (parseInt(app.SPIYearMonth['year__mm']) == YearForWMSSelection) {
                    if (value.slice(4, 6) >= 10) {
                        selectedMonthFileList.push(value);
                    }
                } else {
    ...
    app.SPIYearMonth['year__mm'] = YearForWMSSelection;
    let currentMonthWMS = [];
    selectedMonthFileList.forEach(function (obj) {
...
app.addSPIDekadLayers = async function () {
    ...
    let sortedDekadList = currentDekad.sort();
    let fileNameUptoMonthDekad = currentDekad.slice(-1)[0].slice(0, -5);
    let DekadFileName = currentDekad.filter(function (obj) {
        return obj.slice(0, -5) === fileNameUptoMonthDekad;
    });
    app.SPIYearMonth['year__dd'] = DekadFileName[0].slice(0, 4);
    app.SPIYearMonth['month__dd'] = DekadFileName[0].slice(4, 6);


    //*****************************************************

    let YearForWMSSelection = parseInt(app.SPIYearMonth['year__dd']);
    let range = '';
    if (parseInt(app.SPIYearMonth['month__dd']) < 10) {
        YearForWMSSelection = YearForWMSSelection - 1;
        range = parseInt(app.SPIYearMonth['month__dd']) + 3;
    } else {
        range = parseInt(app.SPIYearMonth['month__dd']) - 9;
    }
    app.SPIYearMonth['range__dd'] = range;
    let selectedFileList = [];
    sortedDekadList.forEach(function (value) {
        let currentYear = parseInt(value.slice(0, 4));
        if (currentYear >= YearForWMSSelection) {
            if (currentYear == parseInt(app.SPIYearMonth['year__dd']) - 1) {
                if (value.slice(4, 6) >= 10) {
                    selectedFileList.push(value);
                }
            } else {
                if (parseInt(app.SPIYearMonth['year__dd']) == YearForWMSSelection) {
                    if (value.slice(4, 6) >= 10) {
                        selectedFileList.push(value);
                    }
                } else {
    ...
    app.SPIYearMonth['year__dd'] = YearForWMSSelection
    let currentDekadWMS = [];
    selectedFileList.forEach(function (obj) {
...
app.addSPI3MonthLayers = async function () {
    ...
    let sorted3Months = current3Months.sort();
    let selected3MonthFile = sorted3Months.slice(-1)[0];
    app.SPIYearMonth['year__3m'] = selected3MonthFile.slice(0, 4);
    app.SPIYearMonth['month__3m'] = selected3MonthFile.slice(4, 6);


    //*****************************************************************
    let YearForWMSSelection = parseInt(app.SPIYearMonth['year__3m']);
    let range = '';
    if (parseInt(app.SPIYearMonth['month__3m']) < 8) {
        YearForWMSSelection = YearForWMSSelection - 1;
        range = parseInt(app.SPIYearMonth['month__3m']) + 5;
    } else {
        range = parseInt(app.SPIYearMonth['month__3m']) - 7;
    }
    app.SPIYearMonth['range__3m'] = range;
    let selectedFileList = [];
    sorted3Months.forEach(function (value) {
        let currentYear = parseInt(value.slice(0, 4));
        if (currentYear >= YearForWMSSelection) {
            if (currentYear == parseInt(app.SPIYearMonth['year__3m']) - 1) {
                if (value.slice(4, 6) >= 8) {
                    selectedFileList.push(value);
                }
            } else {
                if (parseInt(app.SPIYearMonth['year__3m']) == YearForWMSSelection) {
                    if (value.slice(4, 6) >= 8) {
                        selectedFileList.push(value);
                    }
                } else {
    ...
    app.SPIYearMonth['year__3m'] = YearForWMSSelection;
    let current3MonthWMS = [];
    selectedFileList.forEach(function (obj) {
