...
let FewsnetYearly = threddDataSource + '/catalog/fewsnetData/1KM_NEW/catalog.xml';
let workingYear = '';
let AllMonths = {
    0: "January",
    1: "February",
...
app.parseParameters = async function () {
    ...
    if (!app.URLparams['y']) {
        ...
        workingYear = firstLastDate[1];
        app.DEFAULTS.YEAR = workingYear
...
app.createHelpers = function () {
    ...
    app.populateL1 = async function (e) {
        ...
        // $("#selectl0").val(l0);
        let Level1AndLevel2Data = await app.makeRequest('GET', app.API.GEOMSAPI + '?country=' + 'WatershedBasinAfghanistan');
    ...
    app.computeClicked = async function (e) {
        ...
        let ParamObject = {
            ...
            showHideLegend: true,
            series: [{
                name: 'Max Snow Water Equivalent',
                type: 'spline',
                data: ParseResponse.SeriesData.max,
                tooltip: {
                    valueSuffix: ' kg/m<sup>2</sup>'
                },
                color: '#292168'
            }, {
                name: 'Mean Snow Water Equivalent',
                type: 'spline',
                data: ParseResponse.SeriesData.mean,
                tooltip: {
                    valueSuffix: ' kg/m<sup>2</sup>'
                },
                color: '#800080'
            }, {
                name: 'Min Snow Water Equivalent',
                type: 'spline',
                data: ParseResponse.SeriesData.min,
                tooltip: {
                    valueSuffix: ' kg/m<sup>2</sup>'
                },
                color: '#13652a'
        ...
        app.highchartObj = app.highchartObjMeanAndAccumulated(ParamObject);
        app.HCObj = app.DrawChart('chart' + '1', app.highchartObj);
    }
...
//  var zoom = view.getZoom();
    ...
    app.updateGeometry = async function (AdminLevel) {
        let response = await app.makeRequest('GET', '/static/' + TethysAppName + '/Shapes/WatershedBasinAfghanistan/' + AdminLevel + '.geojson')
    ...
    app.APIParameters = function () {
        ...
        let param = {
            "type": "POST",
            "country": 'WatershedBasinAfghanistan',
...
app.getAvailableDataMonth = async function () {
    let currentResponse = await app.makeRequest('GET', FewsnetYearly);
    ...
    for (i of Datasetcollection) {
        let WYyyyy = i.getAttribute('name');
        let YYYY = WYyyyy.slice(2);
        yyyyList.push(YYYY);
    }
    let yyyyListSorted = yyyyList.sort();
    let FirstYear = yyyyListSorted.slice(0)[0]
    let LastYear = yyyyListSorted.slice(-1)[0]
    let FirstLastDate = [parseInt(FirstYear.slice(0, 4)), parseInt(LastYear.slice(0, 4))];
...
app.initiUI = async function () {
    ...
    //         // workingYear = date.slice(1)[0].toString() + date.slice(0)[0].toString();
    //     }
    // });
    let ParseIntDateSelected = parseInt(app.URLparams.y);
    console.log(firstLastDate);
    $("#startDate").yearpicker({
        // Initial Year
        year: ParseIntDateSelected,
        // Start Year
        startYear: firstLastDate[0],
        // End Year
        endYear: firstLastDate[1],
    });
...
app.minMaxUI = function () {
    $('div.ol-overlaycontainer-stopevent').append(`<div id="btn_maxmin_map" full-size="0" size-control="map" class="btn-float shadow-all" onclick="MapToggleSize(this)" style="right:6px;bottom:6px;">
                                <i class="glyphicon glyphicon-chevron-down"></i>
                            </div>`);

    $('#chart1').parent().append(`<div id="btn_maxmin_chart" full-size="0"  size-control="chart" class="btn-float shadow-all" push-target="map-section" pull-target="chart-section" onclick="chartToggleSize(this)" style="left:10px;top:10px;">
                            <i class="glyphicon glyphicon-chevron-up"></i>
                        </div>`);

};

function MapToggleSize(elem) {
    var self = $(elem);
    var isFull = parseInt(self.attr('full-size'));
    var mapEl = $('.map-column');
    if (isFull) {
        changeIcon(self, 'down');
        self.attr('full-size', 0);
        mapEl.height('55%');
        mapEl.children().eq(0).removeClass('padding-bottom-10');
        mapEl.children().eq(0).addClass('padding-bottom-5');
    } else {
        changeIcon(self, 'up');
        self.attr('full-size', 1);
        mapEl.height('100%');
        mapEl.children().eq(0).removeClass('padding-bottom-5');
        mapEl.children().eq(0).addClass('padding-bottom-10');
    }
    MapUpdateAndChartUpdateDOM();
};

function chartToggleSize(elem) {
    var self = $(elem);
    var isFull = parseInt(self.attr('full-size'));
    var chartEl = $('.chart-column');
    var mapEl = $('.map-column');
    if (isFull) {
        changeIcon(self, 'up');
        self.attr('full-size', 0);
        $('.chart-column').appendTo('.map-chart-col');
        chartEl.height('45%');
        chartEl.children().eq(0).removeClass('padding-top-10');
        chartEl.children().eq(0).addClass('padding-top-5');
    } else {
        changeIcon(self, 'down');
        self.attr('full-size', 1);
        $('.chart-column').prependTo('.map-chart-col');
        chartEl.height('100%');
        chartEl.children().eq(0).removeClass('padding-top-5');
        chartEl.children().eq(0).addClass('padding-top-10');
    }
    MapUpdateAndChartUpdateDOM();
};

function MapUpdateAndChartUpdateDOM() {
    app.map.updateSize();
    // var chart1 = $('#chart1').highcharts();
    // chart1.reflow();
    app.DrawChart('chart' + '1', app.highchartObj);
};

function changeIcon(self, dire) {
    if (dire == 'up') {
        $('.glyphicon', self).removeClass('glyphicon-chevron-down');
        $('.glyphicon', self).addClass('glyphicon-chevron-up');
    } else {
        $('.glyphicon', self).addClass('glyphicon-chevron-down');
        $('.glyphicon', self).removeClass('glyphicon-chevron-up');
    }
};
